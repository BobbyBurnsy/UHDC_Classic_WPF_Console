...
# Chrome backup
Wait-TrainingStep `
    -Desc "STEP 1: SECURE CHROME BOOKMARKS`n`nWHEN TO USE THIS:`nUse this before wiping a user's profile, replacing their PC, or resetting a corrupted browser.`n`nWHAT IT DOES:`nWe are accessing the remote computer's hidden C$ administrative share to copy the user's Google Chrome Bookmarks file directly over the network to our local machine. Natively, you can do this using the standard 'copy' command.`n`nIN-PERSON EQUIVALENT:`nIf you were physically at the user's desk, you would open File Explorer, type '%LocalAppData%\Google\Chrome\User Data\Default' into the address bar, copy the file named 'Bookmarks', and save it to a flash drive or network share." `
    -Code "copy `"\\$Target\c$\Users\$TargetUser\AppData\Local\Google\Chrome\User Data\Default\Bookmarks`" `"$destFolder\Chrome_Bookmarks`""
...
# Edge backup
Wait-TrainingStep `
    -Desc "STEP 2: SECURE EDGE BOOKMARKS`n`nWHEN TO USE THIS:`nSame as above, but for Microsoft Edge.`n`nWHAT IT DOES:`nWe are repeating the process for Microsoft Edge, which stores its user data in a similar Chromium-based directory structure.`n`nIN-PERSON EQUIVALENT:`nIf you were physically at the user's desk, you would navigate to '%LocalAppData%\Microsoft\Edge\User Data\Default', copy the 'Bookmarks' file, and save it alongside the Chrome backup." `
    -Code "copy `"\\$Target\c$\Users\$TargetUser\AppData\Local\Microsoft\Edge\User Data\Default\Bookmarks`" `"$destFolder\Edge_Bookmarks`""
...
# Finish and open folder
if ($found) {
    Write-Host "`n [UHDC] Success: Backup complete! Opening local destination folder..."

    Wait-TrainingStep `
        -Desc "STEP 3: VERIFY BACKUP`n`nWHEN TO USE THIS:`nAlways verify you actually have the files before you destroy the originals.`n`nWHAT IT DOES:`nWe are launching File Explorer on your local machine and using the '/select' argument to automatically highlight the newly created backup folder.`n`nIN-PERSON EQUIVALENT:`nIf you were at the user's desk, you would open the flash drive or network share to visually confirm the files were successfully copied before proceeding with any destructive actions (like a profile reset or PC swap)." `
        -Code "explorer.exe /select,`"$destFolder`""
