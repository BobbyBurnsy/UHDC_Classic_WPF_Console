...
# --- Robust Free Space Calculator ---
function Get-TargetFreeSpace ([string]$Comp) {
    # Attempt 1: WinRM / CIM
    try {
        $disk = Get-CimInstance -ComputerName $Comp -ClassName Win32_LogicalDisk -Filter "DeviceID='C:'" -ErrorAction Stop
        return [double]$disk.FreeSpace
    } catch {
        # Attempt 2: PsExec Fallback
        $psExecPath = Join-Path -Path $SharedRoot -ChildPath "Core\psexec.exe"
        if (Test-Path $psExecPath) {
            $wmicOut = & $psExecPath /accepteula \\$Comp -s wmic logicaldisk where "DeviceID='C:'" get FreeSpace /value 2>&1
            foreach ($line in $wmicOut) {
                if ($line -match "FreeSpace=(\d+)") {
...
Write-Host " [UHDC] [i] Calculating current free space..."
$freeBefore = Get-TargetFreeSpace $Target
...
try {
    ...
    Write-Host "  > [i] WinRM blocked. Falling back to PsExec for Temp/Recycle Bin..." -ForegroundColor DarkGray
    if (Test-Path $psExecPath) {
        $cmdChain2 = 'cmd /c "del /q /f /s C:\Windows\Temp\* & rd /s /q C:\$Recycle.Bin"'
        Start-Process $psExecPath -ArgumentList "/accepteula \\$Target -s $cmdChain2" -Wait -NoNewWindow
        Write-Host "  > [OK] System temp and Recycle Bin zeroed out via PsExec."
    }
}

# --- Post-Cleanup Space Check & Calculation ---
Write-Host "`n [UHDC] [i] Calculating recovered disk space..."
$freeAfter = Get-TargetFreeSpace $Target
$freedStr = "0 MB"
...
try {
    ...
    if (Test-Path $psExecPath) {
        Start-Process $psExecPath -ArgumentList "/accepteula \\$Target -s cleanmgr.exe /sagerun:1" -Wait -NoNewWindow
        Write-Host "  > [OK] Disk Cleanup job dispatched via PsExec."
    }
}
